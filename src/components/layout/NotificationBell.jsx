...
import {
  ...
  DollarSign, AlertTriangle, UserX, BookOpen, Package, FileText, Trash2,
} from 'lucide-react';
...
export default function NotificationBell() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      // refresh count from loaded data
      setUnread((Array.isArray(r.data) ? r.data : []).filter(n => !n.is_read).length);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const d = r.data?.data ?? r.data;
      const msg = d?.message || r.data?.message || 'Alerts refreshed';
      toast.success(msg);
      loadAll();
    } catch (e) {
  ...
  return (
    <div className="relative">
      ...
      {/* Dropdown panel — fixed to escape any parent stacking context */}
      {open && (
        <div
          ref={panelRef}
                ...
                {notifications.map(n => {
                  ...
                  return (
                    <li
                      ...
                      className={`flex items-start gap-3 px-4 py-3 border-b border-slate-50 dark:border-slate-800/60 cursor-pointer transition-colors
                        ${n.is_read
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <p className={`text-xs font-semibold text-slate-800 dark:text-slate-100 leading-snug ${!n.is_read ? 'font-bold' : ''}`}>
                            {n.title}
                          </p>
